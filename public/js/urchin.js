...
var _utcp="azeroth.html";			// the cookie path for tracking
...
if (!_utcp || _utcp=="") _utcp="azeroth.html";
...
function _uInfo(page) {
 ...
 if (!_ur || _ur=="") { _ur="-"; }
  ...
  dm=_ubd.domain;
  if(_utcp && _utcp!="azeroth.html") dm+=_utcp;
  p=_ur.indexOf(dm);
  if ((p>=0) && (p<=8)) { _ur="0"; }
...
function _uRef() {
 ...
 if (h.indexOf("azeroth.html") > -1) {
  k=h.substring(h.indexOf("azeroth.html"),h.length);
  if (k.indexOf("?") > -1) k=k.substring(0,k.indexOf("?"));
  h=h.substring(0,h.indexOf("azeroth.html"));
 }
 h=h.toLowerCase();
 n=h;
...
function _uGCse() {
 ...
 h=p=_ur.split("://")[1];
 if(h.indexOf("azeroth.html")>-1) {
  h=h.split("azeroth.html")[0];
  p=p.substring(p.indexOf("azeroth.html")+1,p.length);
 }
 if(p.indexOf("?")>-1) {
  p=p.split("?")[0];
 }
